...
package io.github.muxiaobai.algorithm.StatisticalLearning.DataMining_SVM.libsvm;
import java.io.*;
import java.util.*;
...
class Cache {
	...
	int get_data(int index, float[][] data, int len)
	{
		...
		if(more > 0)
		{
			...
			h.data = new_data;
			size -= more;
			do {int _k=h.len; h.len=len; len=_k;} while(false);
		}
	...
	void swap_index(int i, int j)
	{
		...
		do {float[] _k=head[i].data; head[i].data=head[j].data; head[j].data=_k;} while(false);
		do {int _k=head[i].len; head[i].len=head[j].len; head[j].len=_k;} while(false);
		if(head[i].len > 0) lru_insert(head[i]);
		if(head[j].len > 0) lru_insert(head[j]);

		if(i>j) do {int _k=i; i=j; j=_k;} while(false);
		for(head_t h = lru_head.next; h!=lru_head; h=h.next)
		{
			if(h.len > i)
			{
				if(h.len > j)
					do {float _k=h.data[i]; h.data[i]=h.data[j]; h.data[j]=_k;} while(false);
				else
				{
					// give up
					lru_delete(h);
...
abstract class Kernel extends QMatrix {
	...
	void swap_index(int i, int j)
	{
		do {svm_node[] _k=x[i]; x[i]=x[j]; x[j]=_k;} while(false);
		if(x_square != null) do {double _k=x_square[i]; x_square[i]=x_square[j]; x_square[j]=_k;} while(false);
	}
...
// An SMO io.github.muxiaobai.algorithm in Fan et al., JMLR 6(2005), p. 1889--1918
// Solves:
//
...
class Solver {
	...
	{
		Q.swap_index(i,j);
		do {byte _k=y[i]; y[i]=y[j]; y[j]=_k;} while(false);
		do {double _k=G[i]; G[i]=G[j]; G[j]=_k;} while(false);
		do {byte _k=alpha_status[i]; alpha_status[i]=alpha_status[j]; alpha_status[j]=_k;} while(false);
		do {double _k=alpha[i]; alpha[i]=alpha[j]; alpha[j]=_k;} while(false);
		do {double _k=p[i]; p[i]=p[j]; p[j]=_k;} while(false);
		do {int _k=active_set[i]; active_set[i]=active_set[j]; active_set[j]=_k;} while(false);
		do {double _k=G_bar[i]; G_bar[i]=G_bar[j]; G_bar[j]=_k;} while(false);
	}
...
class SVC_Q extends Kernel
{
	...
	void swap_index(int i, int j)
	{
		...
		do {byte _k=y[i]; y[i]=y[j]; y[j]=_k;} while(false);
		do {float _k=QD[i]; QD[i]=QD[j]; QD[j]=_k;} while(false);
	}
}
...
class SVR_Q extends Kernel
{
	...
	void swap_index(int i, int j)
	{
		do {byte _k=sign[i]; sign[i]=sign[j]; sign[j]=_k;} while(false);
		do {int _k=index[i]; index[i]=index[j]; index[j]=_k;} while(false);
		do {float _k=QD[i]; QD[i]=QD[j]; QD[j]=_k;} while(false);
	}
...
public class svm {
	...
	private static void svm_binary_svc_probability(svm_problem prob, svm_parameter param, double Cp, double Cn, double[] probAB)
	{
		...
		for(i=0;i<prob.l;i++)
		{
			...
			do {int _k=perm[i]; perm[i]=perm[j]; perm[j]=_k;} while(false);
		}
		for(i=0;i<nr_fold;i++)
		{
		    ...
		    param.svm_type == svm_parameter.NU_SVC) && nr_fold < l)
		{
			...
			for (c=0; c<nr_class; c++)
				for(i=0;i<count[c];i++)
				{
					...
					do {int _k=index[start[c]+j]; index[start[c]+j]=index[start[c]+i]; index[start[c]+i]=_k;} while(false);
				}
			for(i=0;i<nr_fold;i++)
			{
				fold_count[i] = 0;
